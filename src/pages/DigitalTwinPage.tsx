...
export default function DigitalTwinPage() {
  ...
  const metrics = useMemo(() => {
    ...
    const baseRevenue = variables.dcRevenue + variables.cgRevenue + variables.embeddedRevenue;
    const baseProfit = dcProfit + cgProfit + embeddedProfit;
    const baseGrossMargin = (baseProfit / baseRevenue) * 100 || 0;

    const priceEffect = 1 + variables.priceAdjustment / 100;
    const demandShiftEffect = 1 + variables.demandShift / 100;
    const elasticityEffect = 1 + variables.priceElasticity * (variables.priceAdjustment / 100);
    const capacityEffect = Math.max(0.5, Math.min(1.2, variables.capacityConstraint / 100));

    const adjustmentFactor = Math.max(0.2, priceEffect * demandShiftEffect * elasticityEffect * capacityEffect);
    const adjustedRevenue = baseRevenue * adjustmentFactor;
    const adjustedProfit = adjustedRevenue * (baseGrossMargin / 100);

    const scaledSegmentData = baseRevenue > 0 ? [
      { name: 'Data Center', revenue: variables.dcRevenue * adjustmentFactor, profit: dcProfit * adjustmentFactor, margin: variables.dcGrossMargin },
      { name: 'Client & Gaming', revenue: variables.cgRevenue * adjustmentFactor, profit: cgProfit * adjustmentFactor, margin: variables.cgGrossMargin },
      { name: 'Embedded', revenue: variables.embeddedRevenue * adjustmentFactor, profit: embeddedProfit * adjustmentFactor, margin: variables.embeddedGrossMargin },
    ] : [];

    return {
      totalRevenue: adjustedRevenue,
      totalGrossProfit: adjustedProfit,
      grossMargin: (adjustedProfit / adjustedRevenue) * 100 || 0,
      segmentData: scaledSegmentData,
    };
  }, [variables]);
