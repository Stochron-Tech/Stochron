...
export function LayerPopup({ layer, isOpen, onClose, variables, updateVariable, metrics }: LayerPopupProps) {
  ...
  return (
    <div className="fixed inset-0 bg-black/40 backdrop-blur-sm flex items-center justify-center z-50 p-4">
      <div className="bg-white rounded-2xl w-full max-w-4xl max-h-[90vh] overflow-y-auto shadow-2xl border-2 border-slate-200">
        {/* Header */}
        ...
        <div className="p-8 grid grid-cols-2 gap-8">
          ...
          <div className={`border-r-2 border-slate-200 pr-8 ${layer === 4 ? 'col-span-2' : ''}`}>
            <h3 className="text-lg font-bold text-slate-900 mb-6">
              {layer === 4 ? 'Outcomes' : 'Variables You Can Adjust'}
            </h3>
                      ...
                      {section.variables.map((varDef) => (
                        <div key={varDef.key}>
                          <div className="flex justify-between items-center mb-2">
                            <label className="text-sm font-medium text-slate-600">{varDef.label}</label>
                            <span className="text-sm font-bold text-slate-900">{variables[varDef.key]}</span>
                          </div>
                          <Slider
          ...
          {layer !== 4 && (
            <div className="pl-8">
              <h3 className="text-lg font-bold text-slate-900 mb-6">Model Structure</h3>
